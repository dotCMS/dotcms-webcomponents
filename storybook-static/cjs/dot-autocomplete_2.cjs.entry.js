...
var autoComplete = _commonjsHelpers.createCommonjsModule(function (module, exports) {
...
const DotAutocompleteComponent = class {
    ...
    render() {
        return (index.h("input", { autoComplete: "off", disabled: this.disabled || null, id: this.id, onBlur: (event) => this.handleBlur(event), onKeyDown: (event) => this.handleKeyDown(event), placeholder: this.placeholder || null }));
    }
    watchThreshold() {
        this.initAutocomplete();
    }
...
const dotChipCss = "dot-chip span{margin-right:0.25rem}dot-chip button{cursor:pointer}";
...
const DotChipComponent = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("span", null, this.label), index.h("button", { type: "button", "aria-label": label, disabled: this.disabled, onClick: () => this.remove.emit(this.label) }, this.deleteLabel)));
    }
    get el() { return index.getElement(this); }
};
DotChipComponent.style = dotChipCss;

exports.dot_autocomplete = DotAutocompleteComponent;
exports.dot_chip = DotChipComponent;
