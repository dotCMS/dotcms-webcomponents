...
const events = require('./events-9b100a64.js');
...
*/
const style = litElement.css `@keyframes mdc-ripple-fg-radius-in{from{animation-timing-function:cubic-bezier(0.4, 0, 0.2, 1);transform:translate(var(--mdc-ripple-fg-translate-start, 0)) scale(1)}to{transform:translate(var(--mdc-ripple-fg-translate-end, 0)) scale(var(--mdc-ripple-fg-scale, 1))}}@keyframes mdc-ripple-fg-opacity-in{from{animation-timing-function:linear;opacity:0}to{opacity:var(--mdc-ripple-fg-opacity, 0)}}@keyframes mdc-ripple-fg-opacity-out{from{animation-timing-function:linear;opacity:var(--mdc-ripple-fg-opacity, 0)}to{opacity:0}}`;
...
limitations under the License.
*/
const supportsCssVariablesWin = foundation.supportsCssVariables(window);
/**
...
const applyRippleStyle = () => {
    ...
    const part = new litElement.NodePart({ templateFactory: litElement.templateFactory });
...
const rippleNode = (options) => {
    ...
    const adapter = {
        ...
        isSurfaceActive: () => foundation.matches(interactionNode, ':active'),
        ...
        registerInteractionHandler: (type, handler) => interactionNode.addEventListener(type, handler, events.applyPassive()),
        deregisterInteractionHandler: (type, handler) => interactionNode.removeEventListener(type, handler, events.applyPassive()),
        registerDocumentInteractionHandler: (evtType, handler) => 
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        document.documentElement.addEventListener(evtType, handler, events.applyPassive()),
        deregisterDocumentInteractionHandler: (evtType, handler) => 
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        document.documentElement.removeEventListener(evtType, handler, events.applyPassive()),
    ...
    const rippleFoundation = new foundation.MDCRippleFoundation(adapter);
 ...
 */
const ripple = litElement.directive((options = {}) => (part) => {
    ...
    if (existingInteractionNode !== undefined &&
        ...
        rippleFoundation = litElement.noChange;
    }
    // make the ripple, if needed
    if (rippleFoundation === litElement.noChange) {
        rippleFoundation =
...
/** @soyCompatible */
class IconButtonBase extends litElement.LitElement {
    constructor() {
        super(...arguments);
        this.disabled = false;
        this.icon = '';
...
IconButton = litElement.__decorate([
    litElement.customElement('mwc-icon-button')
], IconButton);

const dotSelectButtonCss = ".active{color:var(--color-main);pointer-events:none}";
...
const DotSelectButton = class {
    ...
    render() {
        return (index.h(index.Host, null, this.options.map((option) => {
            const active = option.label.toLocaleLowerCase() === this.value.toLocaleLowerCase();
            return (index.h("mwc-icon-button", { class: {
                    active
...
exports.dot_select_button = DotSelectButton;
