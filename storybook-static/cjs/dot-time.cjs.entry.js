...
const DotTimeComponent = class {
    ...
    minWatch() {
        this.min = checkProp.checkProp(this, 'min', 'time');
    }
    maxWatch() {
        this.max = checkProp.checkProp(this, 'max', 'time');
    }
    ...
    render() {
        return (index.h(index.Host, { class: Object.assign({}, this.classNames) }, index.h("dot-label", { label: this.label, required: this.required, name: this.name }, index.h("dot-input-calendar", { "aria-describedby": utils.getHintId(this.hint), tabIndex: this.hint ? 0 : null, disabled: this.disabled, type: "time", name: this.name, value: this.value, required: this.required, min: this.min, max: this.max, step: this.step })), utils.getTagHint(this.hint), this.errorMessageElement));
    }
    validateProps() {
        this.minWatch();
        this.maxWatch();
...
DotTimeComponent.style = dotTimeCss;

exports.dot_time = DotTimeComponent;
